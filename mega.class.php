 ...
 * https://github.com/smartinm/mega-php-client
 */
...
class MEGA {
  ...
  public function setHelperId($id) {
	$this->h_id = $id;
  }
  ...
  protected function file_download_url($url, $size, $key, $dest) {
    ...
    foreach ($chunks as $chunk_start => $chunk_size) {
      ...
      $this->log("Progress " .  sprintf("%01.2f", round(($idxChunk++ / count($chunks)) * 100,2)) . "%");
      while(file_exists(TMP_FOLDER . $this->h_id . "." . PAUSE_EXTENSION)) {
          sleep(1);
      }
    }
